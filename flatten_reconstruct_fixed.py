...
# Flatten and Reconstruct Pandas DataFrames with Spark (Safe Serialization)
import pandas as pd
import numpy as np
import json
...
def flatten_pandas_dataframes(df_list):
    ...
    for df_id, df in enumerate(df_list):
        ...
        index_names = list(df.index.names) if df.index.names else [None]
        column_names = list(df.columns.names) if df.columns.names else [None]
...
def create_spark_dataframe_from_flat(spark: SparkSession, flat_data):
    def serialize_row(row):
        return Row(
            ...
            row_index=json.dumps(row['row_index']),
            col_name=json.dumps(row['col_name']),
            index_names=json.dumps(row['index_names']),
...
def reconstruct_pandas_dataframes_from_spark(spark_df):
    ...
    for df_id in sorted(pd_df['df_id'].unique()):
        ...
        index_names = json.loads(sub_df['index_names'].iloc[0])
        column_names = json.loads(sub_df['column_names'].iloc[0])

        sub_df['row_index'] = sub_df['row_index'].apply(lambda x: tuple(json.loads(x)))
        sub_df['col_name'] = sub_df['col_name'].apply(lambda x: tuple(json.loads(x)))
